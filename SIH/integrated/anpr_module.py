    ...
    def _ocr_worker(self):
        while True:
            try:
                ...
                ocr_result = self.ocr.predict(plate_bgr)
                if ocr_result:
                    for res in ocr_result:
                        ...
                        if texts:
                            plate_info["text"] = texts[0]
                            plate_info["confidence"] = scores[0] if scores else 0.0
                            print(f"[ANPR] Plate: {texts[0]} | Confidence: {plate_info['confidence']:.2f}")
